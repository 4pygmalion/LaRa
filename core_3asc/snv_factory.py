...
class VariantFilter:
    ...
    def _check_known_variant(self, row: pd.Series) -> bool:
        ...
        if row["clinvar_variant:pathogenicity"] != "-":
            ...
            for idx, word in enumerate(row["clinvar_variant:pathogenicity"].split("||")):
                if (
                    word in ["Likely pathogenic", "Pathogenic", "Pathogenic/Likely pathogenic"]
    ...
    def filter_variant(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        cond4 = (
            ((df["exon:region_type"] != "cds") | (df["Consequence"].str.contains("synonymous")))
            & ~(
            ...
            & (df["SPLICEAI-VALUE"].apply(lambda x: float(x) < 0.1 if x != "-" else True))
        )

        cond5 = df["#Uploaded_variation"].str.startswith("MT") & df["Consequence"].str.contains(
            "stream"
        )
...
def read_clinvar_variants(path: str) -> dict:
    ...
    for idx, row in df.iterrows():
        ...
        n_benign = sum(["benign" in pathogenicity.lower() for pathogenicity in pathogenicities])
...
class SNVFeaturizer:
    ...
    def __init__(
        ...
        Example:
            >>> snv_featurizer = SNVFeaturizer(
                    snv_root_path="/data/shared/EVIDENCE.tucuxi.result/result",
        ...
        self.snv_root_path = Path(snv_root_path).resolve()
        self.snv_38_root_path = self.snv_root_path.with_name("result_grch38")
    ...
    def load_inhouse_freq_dict(self, file_path: Union[str, Path]) -> None:
        ...
        Note:
            (WES)3B.ADZQ.variant.count.table.txt
            TOTAL_PATIENT   VALID_PATIENT   TOTAL_VARIANT   26449   26025   19996891
            MALE_N  FEMALE_N        13201   10537
            TOTAL_SAMPLE_WITH_MITOCHONDRIAL_VARIANT 14256
            # Created at 2023-07-17 20:19:46
            # variant       male.het.AC     male.homo.AC    female.het.AC   female.homo.AC  male.total.AN   female.total.AN
            1-10002558-G-A  46      0       32      2       2014    1584
            1-10002558-G-T  1       0       0       0       2014    1584
            1-10002604-C-T  1       0       1       0       3644    2978
            1-10002640-C-T  0       0       1       0       3644    2978

            (WGS) 3B.ADZQ.HG38.variant.count.table.txt
            TOTAL_PATIENT   VALID_PATIENT   TOTAL_VARIANT   10005   10005   122442421
            MALE_N  FEMALE_N        4700    5305
            TOTAL_SAMPLE_WITH_MITOCHONDRIAL_VARIANT 10005
            # Created at 2023-04-10 14:43:36
            # variant       male.het        male.homo       female.het      female.homo
            1-100000223-C-T 417     8       485     13
            1-100000723-G-A 157     2       187     1
            1-100001400-T-G 0       0       1       0
            1-100002782-A-AT        315     3       381     9
            1-100003004-A-G 152     2       180     1

        """
        self.logger.info(f"Load inhouse frequency: {file_path}") if self.logger else None
    ...
    def get_incomplete_zygosity(
        ...
        inheritances_is_nan = (isinstance(inheritances, float)) and (math.isnan(inheritances))
    ...
    def parse_acmg_pipe(self, assigned_rule: pd.Series, delimiter: str = "||") -> pd.Series:
        """구분문자(delimiter)로 연결된 ACMG_RULE을 파싱하여, RULE별 Strength로 변환
    ...
    def featurize_all(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for feature_name, categories in self.evidence_fields["categorical_fields"].items():
    ...
    def annotated_label(self, df: pd.DataFrame, causal_variant: List[Tuple]) -> pd.DataFrame:
        """
        ...
        df["label"] = False
        df.loc[
            df[["#Uploaded_variation", "phenoId"]].apply(tuple, axis=1).isin(causal_variant),
            "label",
        ] = True
    ...
    def annotated_base_feature(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        df["is_incomplete_zygosity"] = df[["SYMBOL_CNT", "inheritance", "vcf_info_GT"]].apply(
            lambda x: self.get_incomplete_zygosity(*x), axis=1
        )
    ...
    def reduce_transcript(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        df.drop_duplicates(["phenoId", "#Uploaded_variation"], keep="first", inplace=True)
    ...
    def build_data(
        ...
        """
        file_path = (
            (self.snv_38_root_path / sample_id / f"{sample_id}.uploaded_data.tsv.gz")
            if sample_id.startswith("G")
            else (self.snv_root_path / sample_id / f"{sample_id}.uploaded_data.tsv.gz")
        )
        ...
        if patient_snv_df is None or any(
            ...
            self.logger.info(f"Tsv file for '{sample_id}' is missing!") if self.logger else None
            return SNVData(causal_variant=causal_variant)

        try:
            patient_snv_df: pd.DataFrame = self.variant_filter.filter_variant(patient_snv_df)
        ...
        if (patient_snv_df.loc[patient_snv_df["label"]]["symptom_similarity"] == "-").any():
        ...
        for func in additional_func:
            feature_df = pd.concat([feature_df, func(patient_snv_df)], ignore_index=True, axis=1)
        ...
        if all_features:
            try:
                ...
                self.logger.info(
                    f"Failed to featurize_all from raw tsv data for '{sample_id}'"
                ) if self.logger else None

                return SNVData(causal_variant=causal_variant)

        feature_df = pd.concat([feature_df.reset_index(drop=True), entire_features], axis=1)
        ...
        return SNVData(
            ...
            variants=[
                Variant(
                    ...
                    acmg_rules=[
                        rule for rule in row["ACMG_RULE"].split("||") if not rule.endswith("-")
                    ],
                    gene_id=row["geneId"],
                )
...
class SNVFeaturizerA4C(SNVFeaturizer):
    ...
    def predict_full_acmg_bayesian_score(
        ...
        rule_onehot = input_dataframe["acmg"].apply(lambda x: self.to_onehot(x)).values  # (N, )
        rule_onehot = np.stack(rule_onehot)  # (N, 6)
        base_acmg_score = input_dataframe["bayesian"].values.astype(np.float32).reshape(-1, 1)
    ...
    def annotate_features(
        ...
        input_dataframe = input_dataframe.sort_values("bayesian", ascending=False).drop_duplicates(
            ["phenoId", "#Uploaded_variation"], keep="first"
        )
        self.logger.debug(
        ...
        # vaf
        input_dataframe["vaf"] = [get_vaf(ad) for ad in input_dataframe["vcf_info_AD"].values]

        # incomplete zygosity
        symbol_count = Counter(input_dataframe["SYMBOL"].values)
        input_dataframe["SYMBOL_CNT"] = input_dataframe["SYMBOL"].apply(lambda x: symbol_count[x])
        ...
        input_dataframe["clinvar_n_benign"] = input_dataframe["#Uploaded_variation"].apply(
            lambda x: self.clinvar_variants.get(x, 0)
        )
    ...
    def build_data_from_df(
        ...
        if missing_fields:
            msg = f"There're several missing fields({missing_fields}) in input dataframe."
        ...
        for _, row in input_dataframe.iterrows():
            ...
            variants.append(
                Variant(
                    ...
                    acmg_rules=[rule for rule in row["acmg"].split("||") if not rule.endswith("-")],
                )
            )
