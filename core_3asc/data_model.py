...
class PatientDataSet:
    ...
    def __post_init__(self):
        ...
        if len(self.data[0].snv_data.x) != 0:
            snv_header = self.data[0].snv_data.header
            n_snv_features = self.data[0].snv_data.x.shape[1]

        if len(self.data[0].cnv_data.x) != 0:
            cnv_header = self.data[0].cnv_data.header
            n_cnv_features = self.data[0].cnv_data.x.shape[1]
        ...
        self.sample_id_to_idx = {p_data.sample_id: i for i, p_data in enumerate(self.data)}
    ...
    def data(self, new_data: List[PatientData]):
        self._data = new_data
        self.sample_id_to_idx = {p_data.sample_id: i for i, p_data in enumerate(new_data)}
